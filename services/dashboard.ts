...
export async function fetchUSCompanyCountFromSupabase(): Promise<{ count: number; error: string | null }> {
  return fetchQuerySuiteCountFromSupabase('total_companies');
}

export async function fetchQuerySuiteCountFromSupabase(queryType: string): Promise<{ count: number; error: string | null }> {
  try {
    const res = await fetch(`/api/supabase-query-suite?type=${encodeURIComponent(queryType)}`, { cache: 'no-store' });
