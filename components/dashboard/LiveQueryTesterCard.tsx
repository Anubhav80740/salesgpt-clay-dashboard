...
const QUERIES: QueryItem[] = [
  {
    ...
    unit: 'Companies',
    description: 'Count of all US companies in company_master',
    icon: Building2,
    color: 'text-blue-600 bg-blue-50 border-blue-100',
    sql: `SELECT COUNT(*) AS total_companies
FROM company_master
WHERE TRIM(LOWER(country)) IN (
  ...
  },
  {
    ...
    unit: 'Tech Companies',
    description: 'Count of US tech companies matched by primary industry array',
    icon: Laptop,
    color: 'text-indigo-600 bg-indigo-50 border-indigo-100',
    sql: `SELECT COUNT(*) AS us_tech_companies
FROM company_master
WHERE TRIM(LOWER(country)) IN (
    'united states', 'us', 'usa', 'united states of america', 'u.s.', 'u.s.a.'
)
AND primary_industry ILIKE ANY (ARRAY[
  ...
  },
  {
    ...
    description: 'Count of US non-tech companies outside tech industry list',
    icon: Briefcase,
    color: 'text-emerald-600 bg-emerald-50 border-emerald-100',
    sql: `SELECT COUNT(*) AS us_non_tech_companies
FROM company_master
WHERE TRIM(LOWER(country)) IN (
    'united states', 'us', 'usa', 'united states of america', 'u.s.', 'u.s.a.'
)
AND NOT (primary_industry ILIKE ANY (ARRAY[
  ...
  },
  {
    ...
    unit: 'Employees',
    description: 'Sum of employee headcount for US tech companies',
    icon: Users,
    color: 'text-purple-600 bg-purple-50 border-purple-100',
    sql: `SELECT SUM(employee_count) AS total_tech_employee_headcount
FROM company_master
WHERE TRIM(LOWER(country)) IN (
    'united states', 'us', 'usa', 'united states of america', 'u.s.', 'u.s.a.'
)
...
AND primary_industry ILIKE ANY (ARRAY[
  ...
  },
  {
    ...
    unit: 'Employees',
    description: 'Sum of employee headcount for US non-tech companies',
    icon: UserCheck,
    color: 'text-amber-600 bg-amber-50 border-amber-100',
    sql: `SELECT COALESCE(SUM(employee_count), 0) AS total_non_tech_employee_headcount
FROM company_master
WHERE TRIM(LOWER(country)) IN (
    'united states', 'us', 'usa', 'united states of america', 'u.s.', 'u.s.a.'
)
...
AND NOT (primary_industry ILIKE ANY (ARRAY[
  ...
  },
  {
    ...
    unit: 'Companies',
    description: 'Count of US companies where employee count is 0 or null',
    icon: UserX,
    color: 'text-rose-600 bg-rose-50 border-rose-100',
    sql: `SELECT COUNT(*) AS us_companies_with_no_employees
FROM company_master
WHERE TRIM(LOWER(country)) IN (
    'united states', 'us', 'usa', 'united states of america', 'u.s.', 'u.s.a.'
)
...
export function LiveQueryTesterCard() {
  ...
  const handleTriggerQuery = async (queryKey: string) => {
    ...
    const { count, error } = await fetchQuerySuiteCountFromSupabase(queryKey);
    ...
    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-soft space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-slate-100">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white rounded-xl text-xs font-bold shadow-sm transition-all shrink-0"
        >
          {isAnyLoading ? (
            <>
              <RefreshCw className="w-4 h-4 animate-spin" />
              <span>Executing All Queries...</span>
            </>
          ) : (
            <>
